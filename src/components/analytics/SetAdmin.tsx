...
import { getUsers, setRole } from "@/services/analytics";
...
const SettingAdmin = () => {
  ...
  const handleSet = async (id: number) => {
    setSelected(id);
    let { status } = await setRole([{ userId: id, status: !admin }]);
    if (status === 200) {
      dispatch(
        setToast({
          type: 2,
          message: `${admin ? "Administrator" : "Player"} "${
            admins.filter((ad) => ad.id === id)[0].name
  ...
  return (
    <div className="w-full">
      <div className="w-full flex justify-between items-center">
        <span className="text-[#717171] font-bold">Set Roles</span>
        <div className="flex gap-2 items-center">
          ...
          <div className="hs-dropdown relative inline-flex !z-30 h-8 bg-transparent rounded-sm border border-[#252525]">
            ...
            >
              <span>{admin ? "Administrator" : "Player"}</span>
            ...
            >
              <button
                ...
                onClick={() => handleFilterAdmin(true)}
              >
                Administrator
              </button>
              <button
                className="flex w-full items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-font hover:bg-[#101010]"
                onClick={() => handleFilterAdmin(false)}
              >
                Player
              </button>
            </div>
            ...
            admins.map((ad, id) => (
              ...
              >
                ...
                <td className="text-sm text-center py-1">
                  {ad.is_admin ? "Administrator" : "Player"}
                </td>
                <td className="text-sm w-32 py-2 px-2">
                  <Button
                    text={ad.is_admin ? "Player" : "Administrator"}
